...
2.  **Creating New Nanopublications:** When declaring your community as a FAIR Implementation Community (FIC) or when selecting FERs keep in mind that:
    ...
    - When the FICs and FERs you are looking for can not be found in the drop-down lists, you’ll have to create new FICs and FERs as nanopublications (go to [Creating nanopublications](https://wiki.gofair.foundation/docs/fip/content.html#creating-nanopublications)).
...
- Click Projects (a folder icon in the menu bar on the left) @fig-dashboard.

![FIP Wizard Dashboard](./_static/img/FIP_dashboard.png){#fig-dashboard fig-align="default" width="100%"}

- You will see a long list of projects on the right (@fig-projectList). FIPs are projects in the FIP Wizard and are normally named by the community that composed them.

![Project List](./_static/img/FIP_projectList.png){#fig-projectList fig-align="default" width="100%"}

- You can create FIPs either from scratch or by cloning an existing FIP, and revising it.

### Clone and adapt the FIP project
<!--
- By default, you can see only projects you created. To see all projects click on the button where you can see your name and click ‘Clear selection’ ().

![Migrate the project if necessary](./_static/img/3.png){fig-align="left" width=80%}
-->
- Locate your reference FIP in the list.

::: {.callout-note}
You can skip the next step, if you don't want to migrate the original project. But you will have to migrate your cloned project in order to publish it.
:::

- If the FIP has an orange annotation ‘update available’ click on the three dots on the right (@fig-update).

  ![Migrate the project if necessary.](./_static/img/FIP_update.png){#fig-update fig-align="default" width="100%"}

  - Select Create Migration, click create and resolve all. This will update your project with the newest Knowledge Model.

- Then click again on the three dots and select Clone (@fig-clone).

  ![Clone the project.](./_static/img/FIP_clone.png){#fig-clone fig-align="default" width="100%"}

- This will bring you to the FIP questionnaire. To change the name of the FIP you need to go to Settings (@fig-newName). You can now start to work with the FIP Wizard questionnaire, and modify the previous FIP declarations (see below).

  ![Change the name of the cloned project.](./_static/img/FIP_newName.png){#fig-newName fig-align="default" width="100%"}

### Make a new FIP

- Click **Create** (blue button on the top menu bar) (@fig-create).

  ![Create a new project.](./_static/img/FIP_create.png){#fig-create fig-align="default" width="100%"}

- You’ll see **Create Project** - this dialog box will launch your new FIP (@fig-template).

  ![Select the project template.](./_static/img/FIP-projectTemplate.png){#fig-template fig-align="default" width="100%"}
...
- Select the **FIP template** from the drop-down list You’ll see here several templates
...
Principally there should be one contact person who acts as a data steward for the community and enters all answers. However, in most cases the answers will need to be agreed upon by an expert team of that community. In any case, it is possible to mention multiple people. By clicking on the **Share button** on the top right you can provide editing or viewing rights for your colleagues (@fig-share).

![Sharing](./_static/img/FIP_share.png){#fig-share fig-align="default" width="100%"}

## Completing the FIP questionnaire

The FIP questionnaire consists of multiple questions (referred to in the FIP ontology as “declarations”), divided among 7 chapters.

    I. About

    II. Metadata for your FAIR Implementation Profile
...
On the left, the FIP Wizard displays a questionnaire navigation tool, indicating the chapters and the number of questions that still need to be answered for each chapter (@fig-navigation).

![Navigation](./_static/img/FIP_navigatgion.png){#fig-navigation fig-align="default" width="100%"}
...
**Chapter II. Metadata for your FAIR Implementation Profile** (@fig-chapterTwo)

This chapter contains these questions:

1.  Identify your FIC (FAIR Implmentation Community) by using the drop-down / autocomplete functions in the text box. If you do not see your community, you’ll have to first create a nanopublication representing your community (see below).

2.  Identify the person or persons who will be responsible for completing the FIP questionnaire using ORCID.
...
5.  Optionally provide a link (OSF or Zenodo DOI) to a case study description, or select from drop down to a case study description created via the wizard you want to focus on in this FIP.

6.  Indicate the version of the FIP. The three positions (1.2.3) stand for different types of changes:

- for a new decision on FSR composition decided by the community;
- for required changes of FSRs related to curation needs;
- for small metadata changes in the FIP. Please do not forget to change the version when you publish the FIP.

![Metadata for your FAIR Implementation Profile](./_static/img/FIP_chapterTwo.png){#fig-chapterTwo fig-align="default" width="100%"}

The F, A, I and R chapters have a series of questions that follow the same format. The explanatory help text throughout these chapters builds upon @Jacobsen2020 and reflects also the GO FAIR Foundation’s interpretations which can be accessed at the GO FAIR Foundation’s website: <https://www.gofair.foundation/interpretation>. This link should help you to understand precisely what type of FAIR Enabling Resource (FER) is being asked for in each case, and for what purpose. Note, in most cases, identical questions are repeated specifically addressing that issue for both data and metadata. In total, there are 12 different FER types, but a FER might be of more than one type (a metadata scheme can also include provenance information and therefore be a FER for {{< var F2 >}} and {{< var R1-2 >}}). In addition to FER types, you can also address Supporting Declarations about FAIR Supporting Resources that are not essential but still helpful in the process of aligning with the FAIR principles. Examples of these are Editors, Crosswalks, and Persistency Policies.
...
- In case your community is not in control of the resource, choose option **a. No Implementation choice** and add the resource as free text in the **Considerations** box (@fig-optionA).

![Option a. No implementation choice](./_static/img/FIP_optionA.png){#fig-optionA fig-align="default" width="100%"}
...
If your community uses a resource that has some level of FAIRness, then click option **b.** and add (declare) the FAIR Enabling Resource (or a list of resources by clicking the **+Add** button) (@fig-optionB).

![Option b.](./_static/img/FIP_optionB.png){#fig-optionB fig-align="default" width="100%"}

- The questionnaire asks you to select (from the drop-down list) a FER for that particular FAIR Principle (@fig-selectFER).

![Select a FER](./_static/img/FIP_selectFER.png){#fig-selectFER fig-align="default" width="100%"}

Note: those items in the list that are marked with the GFF curation logo are approved by the GO FAIR team

![GFF Curation Badge](./_static/img/GFFcurated.svg){fig-align="left" width=20%}
...
![](/docs/img/nanopubstructure.jpg){fig-align="default" width="40%"}
...
- This happens in two ways:
  ...
  - The output of the FIP (see [Publishing your FIP](#sec-fip-publishing)) is an index nanopublication that provides a list of declared FERs for every FIC.
...
![FER Definitions](./_static/img/FERdef.png){fig-align="default" width="100%"}

In addition to FERs, there is the larger typology of FAIR Supporting Resources, that complement essential FERs.

![Typology](/docs/img/FSR Typology1.svg){fig-align="default" width=100%}

A FER might be available or 'to be developed'. For the name of the resource you are creating you should avoid ‘,’ use a pipe (\|) with a space before and after that symbol (for enabling a better ﬁndability) if you want to add both an acronym and a longer name like *Envthes \| Environmental Thesaurus*

5.  After completing all required fields, go to **Documents** (@fig-newDocument) and make a **new document**.

![Create a new document.](./_static/img/FIP_newDocument.png){#fig-newDocument fig-align="default" width="100%"}

6.  For Template choose **Nanopublication Template** and for Format choose **RDF Trig**.

7.  Push **Create**.

    If no error appears you can submit the nanopub by clicking on the three dots (@fig-submit):

    ![Submit](./_static/img/FIP_submit.png){#fig-submit fig-align="default" width="100%"}
...
You will see the submission as a FER nanopub (@fig-viewSub), the persistent identiﬁer (nanopub URI) is highlighted in orange.

![View Submission](/docs/media/171.png){#fig-view-Sub fig-align="default" width="100%"}

This FER will now appear, automatically, in the drop downs of the question where the FER type is expected (e.g. identiﬁer for {{< var F2 >}}).
...
To publish your FIP, use the Actions button which reminds you to increase the version number for a new nanopublication:

![Actions](./_static/img/Actions.png){fig-align="default" width="100%"}
...
You will see the created FIP index nanopub (@fig-indexNano)

![](./_static/img/index_nanopub.png){#fig-indexNano fig-align="default" width="100%"}

If you want to get a human-readable output (@fig-docTemplate) you could choose as document template:

     I. **Questionnaire Report** which gives you the options to create PDFs or Word documents

     II. **FIP Spreadsheet** to get CSV or Excel files (@fig-csv)

![Select the document template.](./_static/img/FIP-docTemplate.png){#fig-docTemplate fig-align="default" width="100%"}

![CSV file](./_static/img/FIP_csvFile.png){#fig-csv fig-align="default" width="100%"}

If an error appears (@fig-error), you should be aware that all questions must have explicit answers: Some questions may have **null** answers, i.e., declarations that no choice was made.

![The latest submission has an error.](./_static/img/FIP_subError.png){#fig-error fig-align="default" width="100%"}

By choosing **HTML Preview** of the **Nanopublication Template** (in **Settings** - @fig-html)

![HTML document format](./_static/img/FIP-docFormat.png){#fig-html fig-align="default" width="100%"}

In **Preview mode** you can see the question that needs revision (@fig-preview).

![Preview shows an error](./_static/img/FIP_Preview.png){#fig-preview fig-align="default" width="100%"}
...
All the nanopublications representing your FIP - i.e., the FIC, the many FERs, and the index nanopub (indeed, all the semantic triples) - will be stored in a dedicated triple store and are available via a [SPARQL endpoint](https://virtuoso.nps.petapico.org/sparql). To get a matrix of all created FIPs, called FIP Matrix (@fig-FIPMatrix) use the [**FIP Matrix tool**](https://fipmatrix.knowledgepixels.com/), which automatically supports comparisons across communities and specific FAIR principles.

![FAIR Convergence Matrix](./_static/img/FAIR%20Matrix.png){#fig-FIPMatrix fig-align="default" width="55%"}

## Revising your FIP

Changes in the FIP are automatically saved.

There is no undo button for any action BUT you can use the version history button (@fig-versionHistory) which allows you to:
...
![Version History](./_static/img/versionHistory.png){#fig-versionHistory fig-align="default" width="100%"}

If you want to answer a specific question later, click on the **plus button** to get a **TODO reminder** (@fig-todo).

![Add a TODO](./_static/img/todo.png){#fig-todo fig-align="default" width="100%"}
